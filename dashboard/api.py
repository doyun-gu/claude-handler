...
from db import get_conn, run_migration, sync_from_json, get_all_tasks, \
    ...
    get_loc_history, update_task_loc, get_tasks_missing_loc
...
async def get_stats_projects_loc():
    ...
    for proj in projects:
        ...
        info = {"name": name, "path": path, "loc": 0, "files": 0, "docs": 0}

        # Total LOC (code files, excluding node_modules, .git, dist, build, __pycache__)
        try:
            r = subprocess.run(
                ["git", "ls-files"],
            ...
            if r.returncode == 0:
                tracked_files = [f for f in r.stdout.strip().split("\n") if f.strip()]
                info["files"] = len(tracked_files)

                # Count docs separately
                doc_exts = {".md", ".rst", ".txt", ".doc", ".docx", ".pdf"}
                info["docs"] = sum(
                    1 for f in tracked_files
                    if any(f.lower().endswith(ext) for ext in doc_exts)
                )

                # Count LOC using git ls-files + wc (fast, only tracked code files)
                code_exts = {
                    ".py", ".js", ".ts", ".tsx", ".jsx", ".rs", ".go", ".java",
                    ".c", ".cpp", ".h", ".hpp", ".cs", ".rb", ".php", ".swift",
                    ".kt", ".scala", ".sh", ".bash", ".zsh", ".css", ".scss",
                    ".less", ".html", ".vue", ".svelte", ".sql", ".yaml", ".yml",
                    ".toml", ".json", ".xml",
                }
                code_files = [
                    f for f in tracked_files
                    if any(f.lower().endswith(ext) for ext in code_exts)
                ...
                # Count lines in batches to avoid arg limit
                total_loc = 0
                batch_size = 200
                for i in range(0, len(code_files), batch_size):
                    ...
                    try:
                        ...
                        if r2.returncode == 0:
                            ...
                            # If multiple files, last line is "total"
                            if len(batch) > 1 and lines:
                                last = lines[-1].strip().split()
                                if last and last[-1] == "total":
                                    total_loc += int(last[0])
                                else:
                                    # Sum individual lines
                                    for line in lines:
        ...
        # LOC contributed by worker tasks (from DB)
        try:
            conn = get_conn()
            row = conn.execute("""
                ...
                FROM tasks
                WHERE project_name = ?
                  AND status IN ('completed', 'merged')
            """, (name,)).fetchone()
            if row:
                info["worker_added"] = row["added"]
                info["worker_removed"] = row["removed"]
        except Exception:
            info["worker_added"] = 0
            info["worker_removed"] = 0
...
async def backfill_loc_stats():
    """Backfill LOC stats for completed tasks by checking git diff on their branches."""
    tasks = get_tasks_missing_loc()
    updated = 0
    ...
    for task in tasks:
        ...
        if not project_path or not branch or not Path(project_path).exists():
            continue

        try:
            # Try to get diff stats: base...branch
            r = subprocess.run(
                ["git", "diff", "--shortstat", f"{base}...{branch}"],
                capture_output=True, text=True, timeout=10, cwd=project_path,
            )
            if r.returncode != 0:
                # Branch might be merged — try from merge-base
                r = subprocess.run(
                    ["git", "log", "--oneline", "--all", f"--grep={task['id']}"],
            ...
            if added > 0 or removed > 0:
                update_task_loc(task["id"], added, removed)
                updated += 1
        except (subprocess.TimeoutExpired, FileNotFoundError):
            pass

    return {"status": "ok", "backfilled": updated, "total_checked": len(tasks)}
